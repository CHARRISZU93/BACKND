...
class Contenedor {
  ...
  /* async getNextId() {
        try {
            let contenido = await knex('bdproductos').select(id)
  ...
  async saveMessage(info) {
    try {
      await this.knex("bdproductos").insert(info);
    } catch (error) {
      console.log(error);
    } finally {
    }
  ...
  async deleteById(id) {
    try {
      ...
      await this.knex.from("bdproductos").where("id", "=", id).del();
  ...
  async deleteAll() {
    try {
      await this.knex.from("bdproductos").del();
    } catch (error) {
      console.log(error);
    } finally {
    }
